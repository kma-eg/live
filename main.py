...
# --- 1. إعداد السيرفر ---
# التعديل هنا: خليته يدور على ملف HTML في نفس الفولدر (.)
...
def handle_callbacks(call):
    ...
    if user_id == str(ADMIN_ID):
        if data == "admin_main":
            ...
            bot.edit_message_text("👮‍♂️ **لوحة التحكم**", chat_id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
            return
        if data == "admin_stats":
            users_count = 0
            ...
            bot.edit_message_text(f"📊 **عدد المستخدمين:** {users_count}", chat_id, call.message.message_id, reply_markup=back, parse_mode="Markdown")
            return
        if data == "admin_broadcast":
            ...
            bot.edit_message_text("📢 أرسل رسالتك الآن للإذاعة:", chat_id, call.message.message_id, reply_markup=back)
            return
        if data == "admin_force":
            curr = open(channel_file).read() if os.path.exists(channel_file) else "لا يوجد"
            admin_states[user_id] = "set_channel"
            back = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="admin_main"))
            bot.edit_message_text(f"🔒 القناة الحالية: {curr}\nأرسل المعرف الجديد أو none للإلغاء:", chat_id, call.message.message_id, reply_markup=back)
            return
...
def admin_actions(message):
    ...
    if state == "broadcast":
        ...
        bot.send_message(message.chat.id, f"✅ تم للإرسال لـ {count}")
        del admin_states[str(message.from_user.id)]
    elif state == "set_channel":
        with open(channel_file, "w") as f: f.write(message.text.strip())
